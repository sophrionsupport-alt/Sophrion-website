...
export default function EventsPage() {
  const [filters, setFilters] = React.useState<FiltersState>({
    q: "",
    status: "all", // reuse FiltersBar status dropdown (mapped below)
    sort: "newest",
  });
  ...
  async function loadEvents() {
    try {
      ...
      // Map FiltersBar status -> events status
      // approved => published, pending => draft, rejected => ignore
  ...
  return (
    <div className="space-y-6">
              ...
              return (
                <>
                  ...
                  <button
                    ...
                    disabled={busy}
                    onClick={() => patchEvent(e.id, { is_published: !e.is_published })}
                  >
                  ...
                  <button
                    ...
                    disabled={busy}
                    onClick={() =>
                      patchEvent(e.id, { registration_open: !e.registration_open })
                    }
                  >
                    {busy ? "Working…" : e.registration_open ? "Close Reg" : "Open Reg"}
                  </button>
      ...
      <div className="text-xs text-foreground/50">
        Note: Filters “Approved/Pending” map to “Published/Draft” for now. If you want,
        we’ll add a dedicated events filter UI next.
      </div>
    </div>
  );
}